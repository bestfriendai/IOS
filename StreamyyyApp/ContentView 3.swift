    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(isPresented: $showingAddStream) {
                AddStreamView(newStreamURL: $newStreamURL, showingAddStream: $showingAddStream)
            }
        }
    }
}
...
struct StreamCardView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            // Stream Preview
            ZStack {
                // Extract channel name from stream URL for Twitch streams
                if stream.type == .twitch, let channelName = extractTwitchChannelName(from: stream.url) {
                    TwitchEmbedWebView(
                        channelName: channelName,
                        chatEnabled: false, // Disable chat in card view
                        quality: .auto,
                        isLoading: .constant(false),
                        hasError: .constant(false),
                        isLive: .constant(stream.isLive),
                        viewerCount: .constant(stream.viewerCount),
                        currentQuality: .constant(.auto),
                        autoplay: true,
                        muted: stream.isMuted,
                        volume: stream.isMuted ? 0.0 : 1.0
                    )
                    .aspectRatio(16/9, contentMode: .fit)
                    .cornerRadius(12)
                    .id(stream.id + "-\(stream.isMuted)") // Force refresh when mute state changes
                } else {
                    // Fallback for non-Twitch streams or invalid URLs
                    Rectangle()
                        .fill(Color.black)
                        .aspectRatio(16/9, contentMode: .fit)
                        .overlay(
                ...
                VStack {
                    ...
                    HStack {
                        // Mute button
                        Button(action: {
                            streamManager.toggleMute(for: stream)
                        }) {
        ...
        .fullScreenCover(isPresented: $showingFullScreen) {
            FullScreenStreamView(stream: stream, showingFullScreen: $showingFullScreen)
        }
    }
    ...
    private func extractTwitchChannelName(from url: String) -> String? {
        ...
        for pattern in patterns {
            if url.lowercased().hasPrefix(pattern) {
                let channelName = String(url.lowercased().dropFirst(pattern.count))
                // Remove any additional path components or query parameters
                let cleanChannelName = channelName.components(separatedBy: "/").first?.components(separatedBy: "?").first ?? ""
                return cleanChannelName.isEmpty ? nil : cleanChannelName
            }
        }
...
struct AddStreamView: View {
    @Binding var newStreamURL: String
    @Binding var showingAddStream: Bool
    @EnvironmentObject var streamManager: StreamManager
        ...
        NavigationView {
            Form {
                Section(header: Text("Stream URL")) {
                    TextField("Enter stream URL", text: $newStreamURL)
                        .textFieldStyle(RoundedBorderTextFieldStyle())
                        .keyboardType(.URL)
                        .autocapitalization(.none)
                }
                
                Section(header: Text("Popular Platforms")) {
                    HStack {
                        Button("Twitch") {
                            newStreamURL = "https://twitch.tv/"
                        }
                        .buttonStyle(.bordered)
                        
                        Button("YouTube") {
                            newStreamURL = "https://youtube.com/"
                        }
                        .buttonStyle(.bordered)
                    }
                }
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarLeading) {
                    Button("Cancel") {
                        showingAddStream = false
                    }
                }
                
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button("Add") {
                        streamManager.addStream(url: newStreamURL)
                        newStreamURL = ""
                        showingAddStream = false
                    }
                    .disabled(newStreamURL.isEmpty)
                }
            }
        }
    }
...
struct FullScreenStreamView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                // Stream content - use TwitchEmbedWebView for Twitch streams
                if stream.type == .twitch, let channelName = extractTwitchChannelName(from: stream.url) {
                    TwitchEmbedWebView(
                        channelName: channelName,
                        chatEnabled: true, // Enable chat in fullscreen
                        quality: .source,
                        isLoading: .constant(false),
                        hasError: .constant(false),
                        isLive: .constant(stream.isLive),
                        viewerCount: .constant(stream.viewerCount),
                        currentQuality: .constant(.source),
                        autoplay: true,
                        muted: false,
                        volume: 1.0,
                        fullscreen: true
                    ...
                    // Fallback for non-Twitch streams
                    ZStack {
                        Rectangle()
                            .fill(Color.black)
